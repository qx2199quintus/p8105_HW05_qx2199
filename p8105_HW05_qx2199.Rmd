...
sim_mu_final = 
  tibble( mu = 0:6) %>% 
  mutate(
    output_mu_other = map(.x = mu, ~rerun (50, problem_3(mu = .x))),
...
```
From the plot, as the effect size, the true mean increases, the proportion of times the null also increases,which indicates that the poewer increases as our effect size increases. 
...
plot2 =
  ...
  )+scale_x_continuous( breaks = seq(0,6,1),limits =c(0,6))+scale_y_continuous( breaks = seq(0,6,1),limits =c(0,6))
